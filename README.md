...
- ポート **2222**: 本物のSSHサーバー（swift-nio-ssh / ed25519ホスト鍵 / パスワード認証）
- ポート **2223**: 素のTCP（認証なし・フォールバック用）
- 行単位の疑似シェル（`help` / `pwd` / `ls` / `cd` / `cat` / `echo` / `device` / `date` / `exit`）
- アプリを**前面に出している間だけ**動く（起動中は自動ロックを抑止）

```sh
./ssh.sh device                  # ラッパー経由（パスワードは .ssh_password）
ssh -T -p 2222 momo@<iPhoneのIP>  # 素のsshでも入れる
```

パスワードは初回起動時にランダム生成してアプリ画面に表示される。
...
| Phase | 内容 | 状態 |
|-------|------|------|
| 1 | TCPサーバー＋疑似シェル、ビルド〜サイドロードのパイプライン確立 | ✅ 実機で動作確認済み |
| 2 | `swift-nio-ssh` で本物のSSHプロトコル対応（`ssh -p 2222` で繋がる） | ✅ 実機で動作確認済み |
| 3 | 写真ライブラリの取り出し（PhotoKit）、公開鍵認証、ファイル転送 | 未着手 |
